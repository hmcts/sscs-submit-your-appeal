...
function setCookiePreference() {
  ...
  setCookie('sscs-tribunals-frontend-cookie-preferences', `{"essential":"on","analytics":${getAnalyticsSelectedValue.value}` ?
    'on' :
    `${'off' +
        ',"apm:"'}${getApmSelectedValue.value}` ?
      'on' :
...
function setAcceptAllCookies() {
  ...
  setCookie('sscs-tribunals-frontend-cookie-preferences', '{"essential":"on","analytics":"on","apm":"on"}', 365);
...
function setRejectAllCookies() {
  ...
  setCookie('sscs-tribunals-frontend-cookie-preferences', '{"essential":"off","analytics":"off","apm":"off"}', 365);
...
function checkCookie() {
  let cookies_policy = getCookie('cookies_policy');
  const cookies_preferences_set = getCookie('cookies_preferences_set');
