...
const {
  getCaseData
} = require('../page-objects/tribunals-case-api/getCaseData');
const {
  appealSubmitConfirmation
} = require('../page-objects/cya/appealSubmitConfirmation');
const {
...
const {
  continueFromStillCanAppeal
} = require('../page-objects/compliance/stillCanAppeal');
const {
  enterReasonForNoMRNAndContinueAfterSignIn
} = require('../page-objects/compliance/noMRN');
const {
...
const {
  editDraftAppeal,
  navigateToDrafts
} = require('../page-objects/draft-appeals/draft-appeals-page');
...
test.describe(`${language.toUpperCase()} - Verifying data when drafts are submitted to CCD`, () => {
  ...
  test(`${language.toUpperCase()} - Sign in and submit draft appeal and verify the submitted CCD @fullFunctional`, async({
    page
  }) => {
    ...
    await selectHaveYouContactedDWPAndContinueAfterSignIn(
      page,
      language,
      commonContent,
      '#haveContactedDWP-yes'
    );
    await enterReasonForNoMRNAndContinueAfterSignIn(
      page,
      language,
      commonContent,
    ...
    await enterAppellantContactDetailsWithMobileAndContinueAfterSignIn(
      page,
      commonContent,
      language
    );
    await checkOptionAndContinueAfterSignIn(
      page,
      commonContent,
    ...
    await addReasonForAppealingUsingTheOnePageFormAfterSignIn(
      page,
      language,
      commonContent,
      testData.reasonsForAppealing.reasons[0]
    );
    await enterAnythingElseAfterSignIn(
      page,
      language,
      commonContent,
      testData.reasonsForAppealing.otherReasons
    );
    await selectAreYouProvidingEvidenceAfterSignIn(
      page,
      language,
      commonContent,
      '#evidenceProvide-no'
    );
    await enterDoYouWantToAttendTheHearingAfterSignIn(
      page,
      language,
      commonContent,
      '#attendHearing-no'
