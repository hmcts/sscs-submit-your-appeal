...
const { confirmDetailsArePresent, enterDetailsFromAttendingTheHearingDatePickerToEnd, enterDetailsFromNoRepresentativeToUploadingEvidence } = require('../page-objects/cya/checkYourAppeal');
const { selectDoYouWantToReceiveTextMessageReminders } = require('../page-objects/sms-notify/textReminders');
const { enterAppellantContactDetailsAndContinue, enterAppellantNINOAndContinue, enterAppellantDOBAndContinue, enterAppellantNameAndContinue } = require('../page-objects/identity/appellantDetails');
...
test.describe(`${language.toUpperCase()} - Appellant does not have a MRN @batch-04`, () => {
  ...
  test(`${language.toUpperCase()} - Appellant has contacted DWP`, async({ page }) => {
    const randomWeekDay = DateUtils.getDateInMilliseconds(DateUtils.getRandomWeekDayFromDate(moment().utc().startOf('day').add(5, 'weeks')));
  ...
  test(`${language.toUpperCase()} - Appellant has not contacted DWP and exits the service`, async({ page }) => {
    await enterBenefitTypeAndContinue(page, language, commonContent, testData.benefitType.code);
    // await chooseLanguagePreference(page, commonContent, testData.languagePreferenceWelsh);
    await enterPostcodeAndContinue(page, language, commonContent, appellant.contactDetails.postCode);
    await checkOptionAndContinue(page, commonContent, '#isAppointee-no');
    await continueFromIndependance(page, commonContent);
