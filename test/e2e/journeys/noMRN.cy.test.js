...
} = require('../page-objects/idam/createAccount');
const {
...
test.describe(`${language.toUpperCase()} - Appellant does not have a MRN @batch-04`, () => {
  ...
  test(`${language.toUpperCase()} - Appellant has contacted DWP`, async({
    ...
    await enterReasonForNoMRNAndContinue(
      page,
      language,
      commonContent,
    ...
    await enterDetailsFromAttendingTheHearingDatePickerToEnd(
      page,
      commonContent,
      language,
      randomWeekDay
  ...
  test(`${language.toUpperCase()} - Appellant has not contacted DWP and exits the service`, async({
    ...
    await expect(page.getByText(contactDWPContent.title)).toBeVisible();
