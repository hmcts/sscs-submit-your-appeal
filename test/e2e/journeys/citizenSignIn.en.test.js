...
test.describe(`${language.toUpperCase()} - Citizen, Sign in scenarios for SYA`, () => {
  ...
  test(`${language.toUpperCase()} - Sign in as a new user and verify draft appeals page @functional`, async({
    page
  }) => {
    await moment().locale(language);
    await enterDetailsFromStartToDraftAppeals(
      page,
      page,
      commonContent,
      language,
      process.env.USEREMAIL_1
    );
    await await enterAppellantContactDetailsWithMobileAndContinueAfterSignIn(
      page,
      commonContent,
      language,
      '07411222222'
    );
    await await checkOptionAndContinueAfterSignIn(
      page,
      commonContent,
    ...
    await await addReasonForAppealingUsingTheOnePageFormAfterSignIn(
      page,
      language,
      commonContent,
      testData.reasonsForAppealing.reasons[0]
    );
    await await enterAnythingElseAfterSignIn(
      page,
      language,
      commonContent,
      testData.reasonsForAppealing.otherReasons
    );
    await await selectAreYouProvidingEvidenceAfterSignIn(
      page,
      language,
      commonContent,
      '#evidenceProvide-no'
    );
    await await enterDoYouWantToAttendTheHearingAfterSignIn(
      page,
      language,
      commonContent,
      '#attendHearing-no'
