...
test.describe(`${language.toUpperCase()} - Appellant PIP, one month ago, attends hearing with dates cannot attend using date-picker @batch-01`, () => {
  ...
  test(`${language.toUpperCase()} - Selects date of when they cannot attend the hearing`, async({
    ...
    ).toHaveText(DateUtils.formatDate(moment(randomWeekDay)));
  });
  ...
  test(`${language.toUpperCase()} - Selects a date when they cannot attend the hearing, then edits the date`, async({
    ...
    const randomWeekDayIn8Weeks = DateUtils.getDateInMilliseconds(
      DateUtils.getRandomWeekDayFromDate(
        moment().utc().startOf('day').add(8, 'weeks')
      )
    );
    ...
    await enterDetailsFromAttendingTheHearingDatePickerToEnd(
      ...
      commonContent,
      language,
      randomWeekDayIn8Weeks
    );
    await expect(
      page.locator('DD MMMM YYYY').first(),
      datesYouCantAttendHearingAnswer
    ).toHaveText(DateUtils.formatDate(moment(randomWeekDayIn8Weeks)));
    ...
    await expect(
      page.locator('DD MMMM YYYY').first(),
      datesYouCantAttendHearingAnswer
    ).toHaveText(DateUtils.formatDate(moment(randomWeekDayIn10Weeks)));
  });
});
