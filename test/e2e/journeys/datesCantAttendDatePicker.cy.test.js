  ...
  () => {
    ...
    test(`${language.toUpperCase()} - Selects date of when they cannot attend the hearing`, async({
      ...
      const randomWeekDay = DateUtils.getDateInMilliseconds(
        DateUtils.getRandomWeekDayFromDate(
          moment().utc().startOf('day').add(10, 'weeks')
        )
      );
      ...
      await expect(
        page
          .getByText(
            DateUtils.formatDate(moment(randomWeekDay), 'DD MMMM YYYY')
          )
          .first()
      ).toBeVisible();
    });
    ...
    test(`${language.toUpperCase()} - Selects a date when they cannot attend the hearing, then edits the date`, async({
      ...
      const randomWeekDayIn9Weeks = DateUtils.getDateInMilliseconds(
        DateUtils.getRandomWeekDayFromDate(
          moment().utc().startOf('day').add(9, 'weeks')
        )
      );
      const randomWeekDayIn11Weeks = DateUtils.getDateInMilliseconds(
        DateUtils.getRandomWeekDayFromDate(
          moment().utc().startOf('day').add(11, 'weeks')
        )
      );
      ...
      await enterDetailsFromAttendingTheHearingDatePickerToEnd(
        ...
        commonContent,
        language,
        randomWeekDayIn9Weeks
      );
      await skipPcq(page);
      await expect(
        page
          .getByText(
            DateUtils.formatDate(moment(randomWeekDayIn9Weeks), 'DD MMMM YYYY')
          )
          .first()
      ).toBeVisible();
      ...
      await expect(
        page
          .getByText(
            DateUtils.formatDate(moment(randomWeekDayIn11Weeks), 'DD MMMM YYYY')
          )
          .first()
      ).toBeVisible();
    });
