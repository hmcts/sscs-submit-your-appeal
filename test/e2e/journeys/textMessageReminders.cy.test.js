...
test.describe(`${language.toUpperCase()} - Appellant PIP, one month ago, does not attend hearing. @batch-05`, () => {
  ...
  test(`${language.toUpperCase()} - Appellant omits optional phone number, does not sign up for text msg reminders.`, async({
    ...
    await expect(page.locator(appellantPhoneNumberAnswer).first()).toHaveText(
      'Not provided'
    );
    await expect(page.locator(receiveTxtMsgRemindersAnswer).first()).toHaveText(
      'No'
    );
  });
  ...
  test(`${language.toUpperCase()} - Appellant omits optional phone number, enters mobile for text msg reminders.`, async({
    ...
    await expect(page.locator(textMsgRemindersMobileAnswer).first()).toHaveText(
      '07455678444'
    );
  });
