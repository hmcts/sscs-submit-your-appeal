...
test.describe(`${language.toUpperCase()} - Terms and Conditions Page @batch-10`, () => {
  ...
  test(`${language.toUpperCase()} - I see the page title text`, async({
    page
  }) => {
    await expect(page.getByText(termsAndConditionsContent.title)).toBeVisible();
  });

  test(`${language.toUpperCase()} - I see expected links and go to expected urls`, async({
    page
  }) => {
    await seeAndGoToGivenLink(page,
