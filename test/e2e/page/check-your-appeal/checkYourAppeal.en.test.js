...
test.describe(`${language.toUpperCase()} - Check-your-appeal @functional`, () => {
  ...
  test(`${language.toUpperCase()} - When page go to the check your appeal page, page don't see the Sign and submit section`, async({ page }) => {
    await enterBenefitTypeAndContinue(page, language, commonContent, 'pip');
    await page.goto(paths.checkYourAppeal);
    await expect(page.getByText(checkYourAppealContent.header).first()).toBeHidden();
  });
});
