...
test.describe(
  ...
  () => {
    ...
    test(`${language.toUpperCase()} - page do not select any checkboxes and continue to see errors`, async({
      ...
      await expect(
        page
          .getByText(hearingArrangementsContent.fields.selection.error.required)
          .first()
      ).toBeVisible();
    ...
    test(`${language.toUpperCase()} - page select a language interpreter, enter a non-existent list language, page see errors`, async({
      ...
      await page
        ...
        .click();
      await page
        .locator(languageInterpreterTextField)
        .first()
        .fill('Invalid language');
      await page
        .getByRole('button', { name: commonContent.continue })
        .first()
        .click();
    ...
    test(`${language.toUpperCase()} - page select sign language and do not enter any language page see errors`, async({
      ...
      await expect(
        page
          .getByText(
            hearingArrangementsContent.fields.selection.signLanguage.language
              .error.required
          )
          .first()
      ).toBeVisible();
    ...
    test(`${language.toUpperCase()} - page select anything else and don't enter any thing into the field page see errors`, async({
      ...
      await expect(
        page
          .getByText(
            hearingArrangementsContent.fields.selection.anythingElse.language
              .error.required
          )
          .first()
      ).toBeVisible();
