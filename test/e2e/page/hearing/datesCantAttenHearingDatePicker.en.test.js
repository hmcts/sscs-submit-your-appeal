...
test.describe(`${language.toUpperCase()} - Dates can't attend date picker @batch-08`, () => {
  ...
  test(`${language.toUpperCase()} - When I go to the page, I see the date-picker`, async({
    page
  }) => {
    await expect(page.locator('#date-picker table').first()).toBeVisible();
  });

  test(`${language.toUpperCase()} - When I select a date, it is highlighted and date is shown in the table`, async({
    page
  }) => {
    await selectDates(page, language, [dateFiveWeeksFromNow]);
  });

  test(`${language.toUpperCase()} - When I select and deselect a date, it isn't highlighted and not shown in the table`, async({
    page
  }) => {
    await selectDates(page, language, [dateFiveWeeksFromNow]);
  ...
  test(`${language.toUpperCase()} - When I select multiple dates, I see them in the table`, async({
    page
  }) => {
    await selectDates(page, language, [
  ...
  test(`${language.toUpperCase()} - When I select multiple dates and them deselect them, I don't see them in the table`, async({
    page
  }) => {
    await selectDates(page, language, [
      dateFiveWeeksFromNow,
    ...
    await deselectDates(page, language, [
      dateFiveWeeksFromNow,
      dateSixWeeksFromNow
    ]);
  });

  test(`${language.toUpperCase()} - When I select a date and then click remove, the date is removed and deselected`, async({
    page
  }) => {
    await selectDates(page, language, [dateFiveWeeksFromNow]);
  ...
  test(`${language.toUpperCase()} - When I select a disabled date, I don't see it in the table`, async({
    page
  }) => {
