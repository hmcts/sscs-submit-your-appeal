...
test.describe(`${language.toUpperCase()} - Send to number`, { tag: '@batch-11' }, () => {
  ...
  test(`${language.toUpperCase()} - When page select Yes, page am taken to the sms confirmation page`, async({ page }) => {
    page.selectUseSameNumberAndContinue(commonContent, '#useSameNumber-yes');
    await page.waitForURL(`**${paths.smsNotify.smsConfirmation}`);
  });

  test(`${language.toUpperCase()} - When page select No, page am taken to the enter mobile page`, async({ page }) => {
