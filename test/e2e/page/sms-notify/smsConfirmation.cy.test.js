...
test.describe(`${language.toUpperCase()} - SMS Confirmation - appellant contact details`, { tag: '@batch-11' }, () => {
  ...
  test(`${language.toUpperCase()} - When page click Continue, page am taken to the Representative page`, async({ page }) => {
  ...
  test(`${language.toUpperCase()} - Enter a mobile and click use same number, page see the number in SMS confirmation`, async({ page }) => {
  ...
  test(`${language.toUpperCase()} - Enter a mobile, click use different number, page see enter mobile number`, async({ page }) => {
    await enterAppellantContactDetailsWithMobileAndContinue(page, commonContent, language, '07466748336');
    await page.getByText(textRemindersContent.fields.doYouWantTextMsgReminders.yes).first().click();
    await page.getByRole('button', { name: commonContent.continue }).first().click();
  ...
  test(`${language.toUpperCase()} - Do not enter a mobile, page see the mobile number page provided for enter mobile`, async({ page }) => {
