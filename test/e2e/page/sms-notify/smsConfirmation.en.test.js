...
test.describe(`${language.toUpperCase()} - SMS Confirmation - appellant contact details @batch-11`, () => {
  ...
  test(`${language.toUpperCase()} - When I click Continue, I am taken to the Representative page`, async({
    page
  }) => {
  ...
  test(`${language.toUpperCase()} - Enter a mobile and click use same number, I see the number in SMS confirmation`, async({
    page
  }) => {
    await enterAppellantContactDetailsWithMobileAndContinue(
      page,
    ...
    await page.waitForURL(`**/${paths.smsNotify.smsConfirmation}`);
    await expect(
      page.getByText(`${smsConfirmationContent.mobileNumber}07466748336`)
    ).toBeVisible();
  });

  test(`${language.toUpperCase()} - Enter a mobile, click use different number, I see enter mobile number`, async({
    page
  }) => {
    await enterAppellantContactDetailsWithMobileAndContinue(
      page,
    ...
    await page.waitForURL(`**/${paths.smsNotify.smsConfirmation}`);
    await expect(
      page.getByText(`${smsConfirmationContent.mobileNumber}+447123456789`)
    ).toBeVisible();
  });

  test(`${language.toUpperCase()} - Do not enter a mobile, I see the mobile number I provided for enter mobile`, async({
    page
  }) => {
    ...
    await page.waitForURL(`**/${paths.smsNotify.smsConfirmation}`);
    await expect(
      page.getByText(`${smsConfirmationContent.mobileNumber}+447987654321`)
    ).toBeVisible();
  });
});
