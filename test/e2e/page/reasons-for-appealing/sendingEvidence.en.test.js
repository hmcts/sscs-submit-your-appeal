...
const { enterAppellantContactDetailsAndContinue, enterAppellantContactDetailsWithEmailAndContinue } = require('../../page-objects/identity/appellantDetails');

test.describe(`${language.toUpperCase()} - Sending Evidence - appellant contact details @evidence-upload`, { tag: '@batch-10' }, () => {
  if (!evidenceUploadEnabled) {
    ...
    test(`${language.toUpperCase()} - When page omit my email address page see the correct content on /sending-evidence`, async({ page }) => {
    ...
    test(`${language.toUpperCase()} - When page add my email address page should see the correct content on /sending-evidence`, async({ page }) => {
    ...
    test(`${language.toUpperCase()} - When page go to the /sending-evidence page page see the title`, async({ page }) => {
      await enterAppellantContactDetailsAndContinue(page, commonContent, language);
      await page.goto(paths.reasonsForAppealing.sendingEvidence);
      await expect(page.getByText(sendingEvidenceContent.title).first()).toBeVisible();
    });

    test(`${language.toUpperCase()} - When clicking continue page see the correct path`, async({ page }) => {
      await enterAppellantContactDetailsAndContinue(page, commonContent, language);
      await page.goto(paths.reasonsForAppealing.sendingEvidence);
