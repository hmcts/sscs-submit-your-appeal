...
test.describe(`${language.toUpperCase()} - Evidence provide page @evidence-upload @batch-10`, () => {
  if (evidenceUploadEnabled) {
    ...
    test(`${language.toUpperCase()} - When I select Yes, I am taken to the evidence upload page`, async({
      page
    }) => {
      ...
      await expect(page.getByText(evidenceUploadContent.title)).toBeVisible();
    });

    test(`${language.toUpperCase()} - When I select No, I am taken to the hearing page`, async({
      page
    }) => {
      await selectAreYouProvidingEvidenceAndContinue(
        page,
