...
test.describe(`${language.toUpperCase()} - Evidence provide page @evidence-upload`, { tag: '@batch-10' }, () => {
  if (evidenceUploadEnabled) {
    ...
    test(`${language.toUpperCase()} - When page select Yes, page am taken to the evidence upload page`, async({ page }) => {
      page.selectAreYouProvidingEvidenceAndContinue(language, evidenceProvideContent.fields.evidenceProvide.yes);
    ...
    test(`${language.toUpperCase()} - When page select No, page am taken to the hearing page`, async({ page }) => {
      page.selectAreYouProvidingEvidenceAndContinue(language, evidenceProvideContent.fields.evidenceProvide.no);
