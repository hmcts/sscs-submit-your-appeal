...
test.describe(`${language.toUpperCase()} - Uploading Evidence @evidence-upload @batch-10`, () => {
  if (evidenceUploadEnabled) {
    ...
    test(`${language.toUpperCase()} - I can upload correctly a file`, async({
      page
    }) => {
    ...
    test(`${language.toUpperCase()} - I cannot upload the wrong type of file`, async({
      page
    }) => {
    ...
    test(`${language.toUpperCase()} - I cannot upload a very large file`, async({
      page
    }) => {
    ...
    test(`${language.toUpperCase()} - I cannot upload more than the total amount of bytes`, async({
      page
    }) => {
    ...
    test(`${language.toUpperCase()} - I see an error if I submit the form without uploading a file`, async({
      page
    }) => {
      await page.click('.govuk-button');
      await expect(page.locator('.govuk-error-summary').first()).toBeVisible();
      await expect(
        page.getByText(evidenceUploadContent.noItemsError)
      ).toBeVisible();
    });
