...
test.describe(
  `${language.toUpperCase()} - Evidence description page @evidence-upload`,
  { tag: '@batch-10' },
  () => {
    if (evidenceUploadEnabled) {
      ...
      test(`${language.toUpperCase()} - When page enter special characters and select continue, page see errors`, async({
        ...
        await expect(
          page.getByText(evidenceDescriptionContent.title).first()
        ).toBeVisible();
        await expect(
          page
            .getByText(
              evidenceDescriptionContent.fields.describeTheEvidence.error
                .invalid
            )
            .first()
        ).toBeVisible();
