  ...
  () => {
    ...
    test(`${language.toUpperCase()} - When page click Continue without adding a reason, page see errors`, async ({
      ...
      await page
        ...
        .click();
      await expect(
        page.getByText(reasonForAppealingContent.listError).first()
      ).toBeVisible();
    });
    ...
    test(`${language.toUpperCase()} - When page go to add another reason and then click Continue without entering any data, page see no errors and am taken to /other-reason-for-appealing`, async ({
      page
    }) => {
      await addAReasonForAppealing(
        page,
    ...
    test(`${language.toUpperCase()} - When page click add Reason multiple times and click Continue without entering any data, page see an error in the error summary`, async ({
      ...
      assert(
        (await page.locator('.govuk-error-summary__list li').all()).length === 1
      );
      await expect(
        page.getByText(reasonForAppealingContent.listError).first()
      ).toBeVisible();
