...
test.describe(
  ...
  () => {
    ...
    test(`${language.toUpperCase()} - When page do not add enough what you disagree with it, page see errors`, async ({
      ...
      await page
        .getByText(reasonsForAppealingContent.links.add)
        .first()
        .click();
      await page
        .locator('textarea[name="item.reasonForAppealing"]')
        .first()
        .fill(reasons[0].reasonForAppealing);
      await page
        .locator('input[name="item.whatYouDisagreeWith"]')
        .first()
        .fill(reasons[3].whatYouDisagreeWith);
      await page
      ...
      await expect(
        page
          .getByText(
            reasonsForAppealingContent.fields.whatYouDisagreeWith.error
              .notEnough
          )
          .first()
      ).toBeVisible();
    ...
    test(`${language.toUpperCase()} - When page do not add enough reason for appealing, page see errors`, async ({
      ...
      await page
        .locator('textarea[name="item.reasonForAppealing"]')
        .first()
        .fill(reasons[3].reasonForAppealing);
      await page
        .locator('input[name="item.whatYouDisagreeWith"]')
        .first()
        .fill(reasons[0].whatYouDisagreeWith);
      await page
        .getByRole('button', { name: commonContent.continue })
        .first()
        .click();
    ...
    test(`${language.toUpperCase()} - When page use whitespace to pad out what you disagree with it, page see errors`, async ({
      ...
      await page
        .locator('input[name="item.whatYouDisagreeWith"]')
        .first()
        .fill(reasons[4].whatYouDisagreeWith);
      await page
        .getByRole('button', { name: commonContent.continue })
        .first()
        .click();
    ...
    test(`${language.toUpperCase()} - When page use whitespace to pad out reason for appealing, page see errors`, async ({
      ...
      await page
        .locator('textarea[name="item.reasonForAppealing"]')
        .first()
        .fill(reasons[4].reasonForAppealing);
      await page
        .locator('input[name="item.whatYouDisagreeWith"]')
        .first()
        .fill(reasons[0].whatYouDisagreeWith);
