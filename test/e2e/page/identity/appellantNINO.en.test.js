...
test.describe(`${language.toUpperCase()} - Appellant NINO form`, { tag: '@batch-09' }, () => {
  ...
  test(`${language.toUpperCase()} - page see the correct information is displayed`, async({ page }) => {
    await expect(page.getByText(appellantNinoContent.title.withoutAppointee).first()).toBeVisible();
    await expect(page.getByText(appellantNinoContent.subtitle.withoutAppointee).first()).toBeVisible();
  });
  ...
  test(`${language.toUpperCase()} - The user has entered a NINO in the wrong format (e.g.AA1234) and continued`, async({ page }) => {
    ...
    await expect(page.getByText(appellantNinoContent.fields.nino.error.required).first()).toBeVisible();
  });
});
