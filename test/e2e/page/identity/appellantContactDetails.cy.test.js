...
test.describe(`${language.toUpperCase()} - Appellant details form`, { tag: '@batch-09' }, () => {
  ...
  test(`${language.toUpperCase()} - When completing the form, clicking Continue, page see url /appellant-text-reminders`, async({ page }) => {
    await page.locator('#addressLine1').first().fill(appellant.contactDetails.addressLine1);
    await page.locator('#addressLine2').first().fill(appellant.contactDetails.addressLine2);
    await page.locator('#townCity').first().fill(appellant.contactDetails.townCity);
    await page.locator('#county').first().fill(appellant.contactDetails.county);
    await page.locator('#postCode').first().fill(appellant.contactDetails.postCode);
    await page.locator('#phoneNumber').first().fill(appellant.contactDetails.phoneNumber);
    await page.locator('#emailAddress').first().fill(appellant.contactDetails.emailAddress);
  ...
  test(`${language.toUpperCase()} - When page click Continue without completing the form page see errors`, async({ page }) => {
    await page.getByRole('button', { name: commonContent.continue }).first().click();
  ...
  test(`${language.toUpperCase()} - When page click Continue with a postcode that is not in England or Wales page see error`, async({ page }) => {
