...
test.describe(`${language.toUpperCase()} - User has an MRN`, { tag: '@batch-07' }, () => {
  ...
  test(`${language.toUpperCase()} - page have an MRN dated one day short of a month ago`, async({ page }) => {
    await enterAnMRNDateAndContinue(commonContent, DateUtils.oneDayShortOfAMonthAgo(page));
    await expect(page).toHaveURL(paths.identity.enterAppellantName);
  });

  test(`${language.toUpperCase()} - page have an MRN dated as the current date`, async({ page }) => {
    await enterAnMRNDateAndContinue(page, commonContent, moment());
    await expect(page).toHaveURL(paths.identity.enterAppellantName);
  });

  test(`${language.toUpperCase()} - page have an MRN dated one month ago`, async({ page }) => {
    await enterAnMRNDateAndContinue(commonContent, DateUtils.oneMonthAgo(page));
    await expect(page).toHaveURL(paths.identity.enterAppellantName);
  });

  test(`${language.toUpperCase()} - page have an MRN dated one month and one day ago`, async({ page }) => {
    await enterAnMRNDateAndContinue(commonContent, DateUtils.oneMonthAndOneDayAgo(page));
    await expect(page).toHaveURL(paths.compliance.checkMRNDate);
  });

  test(`${language.toUpperCase()} - page have an MRN dated one day short of 13 months ago`, async({ page }) => {
    await enterAnMRNDateAndContinue(commonContent, DateUtils.oneDayShortOfThirteenMonthsAgo(page));
    await expect(page).toHaveURL(paths.compliance.checkMRNDate);
  });

  test(`${language.toUpperCase()} - page have an MRN dated 13 months ago`, async({ page }) => {
    await enterAnMRNDateAndContinue(commonContent, DateUtils.thirteenMonthsAgo(page));
    await expect(page).toHaveURL(paths.compliance.checkMRNDate);
  });

  test(`${language.toUpperCase()} - page have an MRN dated 13 months and one day ago`, async({ page }) => {
    await enterAnMRNDateAndContinue(commonContent, DateUtils.thirteenMonthsAndOneDayAgo(page));
    await expect(page).toHaveURL(paths.compliance.checkMRNDate);
  });

  test(`${language.toUpperCase()} - page have a MRN but page do not enter the day, month or the year`, async({ page }) => {
  ...
  test(`${language.toUpperCase()} - When page enter a date that is the same as the date on the image page see errors`, async({ page }) => {
  ...
  test(`${language.toUpperCase()} - page enter a MRN date with name of month`, async({ page }) => {
    date.month = 'ocToBer';
    await enterADateAndContinue(page, commonContent, date.day, date.month, date.year);
    await expect(page).toHaveURL(paths.compliance.checkMRNDate);
  });

  test(`${language.toUpperCase()} - page enter a MRN date with the short name of month`, async({ page }) => {
    date.month = 'aUg';
    await enterADateAndContinue(page, commonContent, date.day, date.month, date.year);
    await expect(page).toHaveURL(paths.compliance.checkMRNDate);
  });

  test(`${language.toUpperCase()} - page enter a MRN date with an invalid name of month`, async({ page }) => {
