...
test.describe(`${language.toUpperCase()} - MRN Over thirteen months late`, { tag: '@batch-07' }, () => {
  ...
  test(`${language.toUpperCase()} - page enter a lateness reason, page click continue, page am taken to /enter-appellant-name`, async({ page }) => {
  ...
  test(`${language.toUpperCase()} - MRN is over 13 months late, page omit a reason why my appeal is late, page see errors`, async({ page }) => {
    await page.getByRole('button', { name: commonContent.continue }).first().click();
  ...
  test(`${language.toUpperCase()} - page enter a reason why my appeal is late, it is less than five chars, page see errors`, async({ page }) => {
  ...
  test(`${language.toUpperCase()} - page enter a reason why my appeal is late with a special character, page see errors`, async({ page }) => {
    await page.locator('#reasonForBeingLate').fill('<Reason for being late>');
    await page.getByRole('button', { name: commonContent.continue }).first().click();
    await expect(page).toHaveURL(paths.compliance.mrnOverThirteenMonthsLate);
