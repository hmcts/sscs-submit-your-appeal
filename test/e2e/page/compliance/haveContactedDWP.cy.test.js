...
test.describe(`${language.toUpperCase()} - Have Contacted DWP`, { tag: '@batch-07' }, () => {
  ...
  test(`${language.toUpperCase()} - When page select yes page am taken to the No MRN page`, async({ page }) => {
    page.selectHaveYouContactedDWPAndContinue(language, commonContent, '#haveContactedDWP-yes');
    await page.waitForURL(`**${paths.compliance.noMRN}`);
  });

  test(`${language.toUpperCase()} - When page select no page am taken to the contact DWP page`, async({ page }) => {
    page.selectHaveYouGotAMRNAndContinue(language, commonContent, '#haveContactedDWP-no');
