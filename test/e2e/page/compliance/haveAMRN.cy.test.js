...
test.describe(`${language.toUpperCase()} - Check MRN @batch-07`, () => {
  ...
  test(`${language.toUpperCase()} - When I select yes I am taken to the DWP Issuing office page`, async({
    page
  }) => {
  ...
  test(`${language.toUpperCase()} - When I select no I am taken to the have you contacted DWP page`, async({
    page
  }) => {
    await selectHaveYouGotAMRNAndContinue(
      page,
  ...
  test(`${language.toUpperCase()} - When I click continue without selecting an option, I see an error`, async({
    ...
    await expect(
      page.getByText(haveAMRNContent.fields.haveAMRN.error.required)
    ).toBeVisible();
  });
});
