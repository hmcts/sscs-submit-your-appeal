...
test.describe(`${language.toUpperCase()} - Enter postcode`, () => {
  ...
  test(`${language.toUpperCase()} - When page go to the enter postcode page page see the page heading`, async({ page }) => {
    await expect(page.getByText(postcodeCheckerContent.title).first()).toBeVisible();
  });

  test(`${language.toUpperCase()} - When entering a postcode in England, page go to the /are-you-an-appointee page`, async({ page }) => {
  ...
  test(`${language.toUpperCase()} - When page enter a postcode in Scotland, page go to the /invalid postcode page`, async({ page }) => {
    await page.locator('#postcode').fill('EH8 8DX');
    await page.getByRole('button', { name: commonContent.continue }).first().click();
    await page.waitForURL(`**${paths.start.invalidPostcode}`);
  });

  test(`${language.toUpperCase()} - When page enter an invalid postcode page see an error`, async({ page }) => {
  ...
  test(`${language.toUpperCase()} - When page leave the postcode field empty and continue, page see an error`, async({ page }) => {
