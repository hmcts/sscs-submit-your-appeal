...
export class AppellantContactDetailsPage extends BasePage {
  ...
  async enterContactDetails(appellantAddressAndContactDetails: any) {
    await this.page.getByRole('textbox', { name: 'Enter a postcode in England,' }).fill(appellantAddressAndContactDetails.postCode);
    await this.page.getByRole('button', { name: 'Find address' }).click();
    await this.page.waitForLoadState('networkidle');
    await this.page.getByLabel('Pick an address').selectOption(appellantAddressAndContactDetails.addressOption);
    await this.page.waitForLoadState('networkidle');
