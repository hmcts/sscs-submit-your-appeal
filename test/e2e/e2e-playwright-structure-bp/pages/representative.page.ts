...
export class RepresentativePage extends BasePage {
  ...
  async requireLegalRep(option: boolean) {
    option
      ? await this.page
          .getByText('Yes, I want to register a representative')
          .click()
      : await this.page
          .getByText('No, I don’t want to register a representative')
          .click();
  ...
  async enterLegalRepDetails(lrDetails: any) {
    await this.page
      ...
      .fill(lrDetails.firstName);
    await this.page
      .getByRole('textbox', { name: 'Last name(s)' })
      .fill(lrDetails.lastName);
    await this.page
      .getByRole('textbox', { name: 'Organisation (if they work' })
      .fill(lrDetails.organisation);
  }
  ...
  async enterLegalRepContactDetailsManual(lrDetails: any) {
    ...
    await this.page
      ...
      .fill(lrDetails.contactDetails.addressLine1);
    await this.page
      .locator('#addressLine2')
      .fill(lrDetails.contactDetails.addressLine2);
    await this.page
      .locator('#townCity')
      .fill(lrDetails.contactDetails.townCity);
    await this.page.locator('#county').fill(lrDetails.contactDetails.county);
    await this.page
      .locator('#postCode')
      .fill(lrDetails.contactDetails.postCode);
    await this.page
      .getByRole('textbox', { name: 'Phone number (optional)' })
      .fill(lrDetails.contactDetails.phoneNumber);
    await this.page
      .getByRole('textbox', { name: 'Email address (optional)' })
      .fill(lrDetails.contactDetails.emailAddress);
    await this.submitPage();
  }
}
