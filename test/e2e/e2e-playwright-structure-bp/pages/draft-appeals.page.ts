...
export class DraftAppealsPage extends BasePage {
  defaultPageContent: any = {
    ...
    "bodyContents": [
      ...
      "If you no longer wish to submit any of these applications for appeal, please select ‘archive’ next to the appropriate application.",
      "If you wish to submit a new appeal, please select ‘create new application’ to provide your appeal information."
    ]
  };

  async editAppeal(){
    await this.page.getByRole('row', { name: 'Infected' }).getByRole('link').first().click();
  }

  async archiveAppeal(){
    await this.page.getByRole('row', { name: 'Infected' }).getByRole('link').last().click();
  }
