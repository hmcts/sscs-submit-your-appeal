...
async function verifyDraftAppealsAndArchiveACase(I) {
  ...
  await expect(I.getByText('Are you sure you want to archive your appeal application?').first()).toBeVisible();
...
async function editDraftAppeal(I, language) {
  await expect(I.locator(`.form-buttons-group [href='${paths.newAppeal}']`).first()).toBeVisible();
  if (language === 'en') {
    await expect(I.getByText('Your draft benefit appeals').first()).toBeVisible();
    await expect(I.getByRole('link', { name: 'Edit' }).first()).toBeVisible();
    await expect(I.getByRole('link', { name: 'Archive' }).first()).toBeVisible();
  ...
  const ccdCaseIDLocators = await I.locator('.govuk-table__cell:nth-child(1)').all();
  const ccdCaseIDs = await Promise.all(ccdCaseIDLocators.map(locator => locator.innerText()));
  const ccdCaseID = Array.isArray(ccdCaseIDs) ? ccdCaseIDs[0] : ccdCaseIDs;

  await I.locator(`[href='${paths.editDraft}?caseId=${ccdCaseID}']`).first().click();
