...
} = require('../upload-evidence/evidenceProvide');
...
} = require('../reasons-for-appealing/sendingEvidence');
const {
  enterAnythingElseAndContinue
} = require('../reasons-for-appealing/reasonsForAppealing');
const {
  addReasonForAppealingUsingTheOnePageFormAndContinue
} = require('../reasons-for-appealing/reasonForAppealingOnePageForm');
const {
...
const {
  ...
  selectDates
} = require('../hearing/datesCantAttend');
...
} = require('../sign-in/sign-in-with-valid-creds');
const {
  createNewApplication
} = require('../draft-appeals/create-new-application');
const { signOut } = require('../sign-out/sign-out');
const { createTheSession } = require('../session/createSession');
const {
  uploadAPieceOfEvidence
} = require('../upload-evidence/uploadEvidencePage');
...
async function enterDetailsFromStartToNINO(
  I,
  commonContent,
  language,
  ...
  await chooseLanguagePreference(
    I,
    language,
    commonContent,
  ...
  if (allowSaveAndReturnEnabled) {
    await selectIfYouWantToCreateAccount(
      I,
      language,
      commonContent,
      '#createAccount-2'
  ...
  await enterAnMRNDateAndContinue(
    I,
    commonContent,
    DateUtils.oneMonthAgo(language)
  );
  await enterDWPIssuingOfficeAndContinue(
    I,
    commonContent,
    testDataEn.mrn.dwpIssuingOffice
  );
  ...
  await enterAppellantNINOAndContinue(
    ...
    language,
    commonContent,
    testNIData.generateValidNINumber()
  );
}

async function enterCaseDetailsFromStartToNINO(
  I,
  commonContent,
  language,
  benefitTypeCode,
  ...
  await enterAnMRNDateAndContinue(
    I,
    commonContent,
    DateUtils.getRandomDateInLast30Days(language)
  );
...
async function enterDetailsFromStartToDraftAppeals(
  ...
  await enterBenefitTypeAfterSignIn(
    I,
    language,
    commonContent,
    benefitTypeCode
  ...
  await continueFromIndependance(I, commonContent);
  await selectHaveYouGotAMRNAndContinueAfterSignIn(
    I,
    language,
    commonContent,
    '#haveAMRN'
  );
  await enterAnMRNDateAndContinueAfterSignIn(
    I,
    commonContent,
    DateUtils.oneMonthAgo(language)
  );
  await enterDWPIssuingOfficeAndContinueAfterSignIn(
    I,
    commonContent,
    testDataEn.mrn.dwpIssuingOffice
  );
  await selectAreYouAnAppointeeAndContinueAfterSignIn(
    I,
    language,
    commonContent,
    '#isAppointee'
  ...
  await enterAppellantNINOAndContinueAfterSignIn(
    I,
    language,
    commonContent,
    testNIData.generateValidNINumber()
...
async function enterDetailsFromStartToDraft(
  ...
  await signInVerifylanguage(
    I,
    newUserEmail,
    testDataEn.signIn.password,
    language
  );
  await I.goto(`${paths.drafts}?lng=${language}`);
  await createNewApplication(I, language);
  ...
  await chooseLanguagePreferenceAfterSignIn(
    I,
    language,
    commonContent,
    '#languagePreferenceWelsh'
  );
  await enterPostcodeAndContinueAfterSignIn(
    I,
    language,
    commonContent,
    appellant.contactDetails.postCode
...
async function enterDetailsFromNoRepresentativeToUploadingEvidence(
  ...
  await enterAnythingElseAndContinue(
    ...
    language,
    commonContent,
    testDataEn.reasonsForAppealing.otherReasons
  );
...
async function enterDetailsFromNoRepresentativeToNoUploadingEvidence(
  I,
  language,
  commonContent
) {
...
async function enterDetailsFromNoRepresentativeToEnd(
  I,
  language,
  commonContent
) {
...
async function enterDetailsFromAttendingTheHearingToEnd(
  I,
  commonContent,
  language,
  date
...
async function enterDetailsFromAttendingTheHearingDatePickerToEnd(
  I,
  commonContent,
  language,
  date
...
async function enterDetailsFromAttendingTheHearingWithSupportToEnd(
  I,
  commonContent,
  language,
  options,
...
async function confirmDetailsArePresent(I, language, hasMRN = true, mrnDate) {
  ...
  // Type of benefit
  await expect(
    I.getByText(testData.benefitType.description).first()
  ).toBeVisible();
  ...
  if (hasMRN) {
    ...
    await expect(
      I.getByText(
        ...
        selectors[language].mrn.dwpIssuingOffice
      ).first()
    ).toBeVisible();

    // The Date of the MRN
    await expect(
      I.getByText(DateUtils.formatDate(mrnDateToCheck, 'DD MMMM YYYY')).first()
    ).toBeVisible();

    if (mrnDateToCheck.isAfter(oneMonthAgo)) {
      // Reason why the MRN is late
      await expect(
        I.getByText(testData.mrn.reasonWhyMRNisLate).first()
      ).toBeVisible();
    }
  } else {
    // Reason for no MRN
    await expect(
      I.getByText(
        testData.mrn.reasonForNoMRN,
        selectors[language].mrn.noMRN
      ).first()
    ).toBeVisible();
  }
  ...
  await expect(
    I.getByText(
      `${appellant.title}. ${appellant.firstName} ${appellant.lastName}`
    ).first()
  ).toBeVisible();
  ...
  // Appellant address
  await expect(
    I.getByText(appellant.contactDetails.addressLine1).first()
  ).toBeVisible();
  if (appellant.contactDetails.addressLine2 !== '') {
    await expect(
      I.getByText(appellant.contactDetails.addressLine2).first()
    ).toBeVisible();
  }
  await expect(
    I.getByText(appellant.contactDetails.townCity).first()
  ).toBeVisible();
  await expect(
    I.getByText(appellant.contactDetails.county).first()
  ).toBeVisible();
  await expect(
    I.getByText(appellant.contactDetails.postCode).first()
  ).toBeVisible();
  ...
  await expect(
    I.getByText(testData.reasonsForAppealing.otherReasons).first()
  ).toBeVisible();

  // Shows when the appeal is complete
  await expect(
    I.getByText(checkYourAppealContent.header).first()
  ).toBeVisible();
}
