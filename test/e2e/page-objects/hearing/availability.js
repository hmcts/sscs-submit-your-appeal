...
async function selectHearingAvailabilityAndContinue(I, language, commonContent, option) {
  const hearingAvailabilityContent = language === 'en' ? hearingAvailabilityContentEn : hearingAvailabilityContentCy;

  await expect(I.getByText(hearingAvailabilityContent.title).first()).toBeVisible();
