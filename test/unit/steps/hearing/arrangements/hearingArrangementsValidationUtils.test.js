...
const { languageInList, emptyLanguageFieldValidation, validCharacters, optionSelected } = require('steps/hearing/arrangements/hearingArrangementsValidationUtils');
...
describe('hearingArrangementsValidationUtils.js', () => {
    ...
    describe('validCharacters', () => {
        ...
        it('returns false when requested field has been set but language field has invalid characters', () => {
            value.language = '<invalid characters>';
            expect(validCharacters(value)).to.equal(false);
        });

        it('returns true when requested field has been set and language field has valid characters', () => {
